import React from 'react'

export default function Choose() {
  return (
    <div id="choose">
            <div
  className="animate in-view is-animated"
  data-delay="0"
  data-duration="400"
  data-animation="fade-up"
>
  <div className="section">
    <div className="shell shell--medium">
      <div className="section__head">
        <h6>Twicsy's Instagram Packages</h6>
        <h2>
          Choose the Way You Want to Grow: <br />
          Real Instagram Views, Followers, Likes!
        </h2>
        <p>
          All Instagram marketing campaigns are different, and that's why we
          give you a wide variety of options to choose from.
        </p>
      </div>
      <div className="callout callout--cyan callout--small">
        <div className="callout__image">
          <i className="svg-blob svg-blob--blobPhone"
            ><svg
              width="103"
              height="101"
              viewBox="0 0 103 101"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M71.3998 77.7153C76.3721 73.6779 79.2107 67.5775 79.0969 61.1735L78.5226 28.8568C78.3182 17.3537 68.8274 8.1944 57.3243 8.39882C45.8213 8.60325 36.662 18.094 36.8664 29.5971L36.9679 35.3104L37.3032 78.1227L53.6301 81.852C59.8743 83.2783 66.4275 81.7527 71.3998 77.7153Z"
                fill="url(#paint0_linear_47_1771)"
              ></path>
              <path
                d="M71.3998 77.7153C76.3721 73.6779 79.2107 67.5775 79.0969 61.1735L78.5226 28.8568C78.3182 17.3537 68.8274 8.1944 57.3243 8.39882C45.8213 8.60325 36.662 18.094 36.8664 29.5971L36.9679 35.3104L37.3032 78.1227L53.6301 81.852C59.8743 83.2783 66.4275 81.7527 71.3998 77.7153Z"
                fill="#91EAF0"
              ></path>
              <path
                d="M51.353 19.5227C52.6932 22.2366 55.871 21.7274 56.7568 19.5005M59.4811 19.3311C60.8213 22.045 63.9991 21.5358 64.8849 19.309M49.9385 24.7844C55.7485 31.2932 62.2478 29.8758 66.1684 23.0757"
                stroke="#0C7C84"
                stroke-width="1.13296"
                stroke-linecap="round"
                stroke-linejoin="round"
              ></path>
              <rect
                x="70.6993"
                y="19.1041"
                width="11.6769"
                height="20.7"
                rx="3.4811"
                transform="rotate(16.165 70.6993 19.1041)"
                fill="#0C7C84"
              ></rect>
              <rect
                opacity="0.4"
                x="67.1367"
                y="46.0889"
                width="12.2077"
                height="29.5156"
                rx="6.10386"
                transform="rotate(-8.3767 67.1367 46.0889)"
                fill="#0C7C84"
              ></rect>
              <rect
                x="69.2581"
                y="38.6539"
                width="12.2077"
                height="29.5156"
                rx="6.10386"
                transform="rotate(-8.3767 69.2581 38.6539)"
                fill="#91EAF0"
              ></rect></svg
          ></i>
        </div>
        <div className="callout__content">
          <div className="flex-row lg-cols-1">
            <div className="flex-col">
              <p>
                And if you want to implement a specialized Instagram marketing
                strategy, reach out to us. We love special orders!
              </p>
            </div>
          </div>
        </div>
      </div>
      <div className="services">
        <ul>
          <li>
            <div className="service text-sm service--cyan">
              <div className="service__inner">
                <div className="service__entry">
                  <h3>Instagram Likes Services</h3>
                  <p>
                    Our Instagram likes packages instantly make your posts more
                    popular. And a high number of IG likes mushrooms quickly
                    into more and more likes. Choose high-quality or premium
                    likes; both are incredibly inexpensive.
                  </p>
                </div>
                <div className="offer has-stars">
                  <a className="btn" href="/buy-instagram-likes"
                    ><span></span><span></span><span></span
                    ><strong>Buy Likes</strong></a
                  >
                  <div className="offer__inner">
                    <p>
                      Starting at <strong>$1.47</strong>
                    </p>
                    <div className="stars text-xsm">
                      <ul>
                        <li>
                          <svg
                            className="svg-icon svg-icon--star"
                            width="19"
                            height="20"
                            viewBox="0 0 19 20"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                          >
                            <path
                              d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                              fill="#FF553C"
                            ></path>
                          </svg>
                        </li>
                        <li>
                          <svg
                            className="svg-icon svg-icon--star"
                            width="19"
                            height="20"
                            viewBox="0 0 19 20"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                          >
                            <path
                              d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                              fill="#FF553C"
                            ></path>
                          </svg>
                        </li>
                        <li>
                          <svg
                            className="svg-icon svg-icon--star"
                            width="19"
                            height="20"
                            viewBox="0 0 19 20"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                          >
                            <path
                              d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                              fill="#FF553C"
                            ></path>
                          </svg>
                        </li>
                        <li>
                          <svg
                            className="svg-icon svg-icon--star"
                            width="19"
                            height="20"
                            viewBox="0 0 19 20"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                          >
                            <path
                              d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                              fill="#FF553C"
                            ></path>
                          </svg>
                        </li>
                        <li>
                          <svg
                            className="svg-icon svg-icon--star"
                            width="19"
                            height="20"
                            viewBox="0 0 19 20"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                          >
                            <path
                              d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                              fill="#FF553C"
                            ></path>
                          </svg>
                        </li>
                      </ul>
                    </div>
                    <div className="stars text-xsm">
                      <ul></ul>
                      <strong className="text-orange"
                        >5
                        / 5</strong
                      >
                      — based on
                      6
                      reviews
                    </div>
                  </div>
                </div>
                <svg
                  className="svg-icon svg-icon--shield"
                  width="40"
                  height="49"
                  viewBox="0 0 40 49"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <ellipse
                    cx="20.5"
                    cy="20.0078"
                    rx="15.5"
                    ry="15"
                    fill="#FF553C"
                  ></ellipse>
                  <path
                    d="M20 0.0078125L0 5.83327V14.7792C0 30.4512 7.38909 38.455 20 48.0078C32.611 38.4493 40 30.4512 40 14.7792V5.83327L20 0.0078125ZM17.9677 32.1714L8.16168 22.5862L12.4318 18.3476L17.9616 23.7526L29.3834 12.5861L33.6592 16.8246L17.9677 32.1714Z"
                    fill="#FFB6AB"
                  ></path>
                </svg>
              </div>
              <div className="blobs blobs--cyan">
                <div className="blobs__holder">
                  <div className="blob blob--left">
                    <svg
                      width="274"
                      height="238"
                      viewBox="0 0 274 238"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        className="blob__body"
                        d="M143.663 232.92C121.673 233.098 100.832 223.313 87.1516 206.389L18.114 120.983C-6.45971 90.5833 -1.31022 46.3171 29.6158 22.1119C60.5418 -2.09334 105.533 2.92843 130.107 33.3283L142.312 48.4274L234.899 160.655L199.78 205.48C186.349 222.623 165.654 232.743 143.663 232.92Z"
                        fill="#FFB6AB"
                      ></path>
                      <path
                        className="blob__face"
                        d="M41.3622 63.1121C50.1713 68.102 58.0068 60.72 56.272 52.8938M63.4911 47.3046C72.3002 52.2945 80.1356 44.9125 78.4008 37.0863M37.5951 78.3983C65.9042 85.4868 95.6536 65.5285 93.7198 39.3447"
                        stroke="#FF553C"
                        stroke-width="4.92264"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                    </svg>
                  </div>
                  <div className="blob blob--middle">
                    <svg
                      width="349"
                      height="340"
                      viewBox="0 0 349 340"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        className="blob__body"
                        d="M105.043 246.657C87.2405 232.715 77.1338 229.278 76.529 207.517L73.7357 104.671C72.6492 65.5833 104.002 34.8709 143.764 36.0731C183.526 37.2753 216.64 69.9369 217.726 109.025L218.007 121.47L222.873 249.546L166.265 240.452C144.921 245.024 122.846 260.599 105.043 246.657Z"
                        fill="#FFB6AB"
                      ></path>
                      <path
                        className="blob__face"
                        d="M125.413 62.6623C128.746 69.987 137.234 68.9011 139.775 63.0579M147.029 62.8369C150.362 70.1616 158.85 69.0758 161.391 63.2326M111.251 68.8341C126.143 86.6195 165.657 85.5081 176.648 67.7673"
                        stroke="#FF553C"
                        stroke-width="3.91287"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                    </svg>
                  </div>
                  <div className="blob blob--right">
                    <svg
                      width="267"
                      height="234"
                      viewBox="0 0 267 234"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        className="blob__body"
                        d="M133.391 229.998C154.784 228.858 174.464 217.94 186.755 200.392L248.779 111.841C270.856 80.3216 263.202 36.8729 231.682 14.7958C200.163 -7.28138 156.714 0.373153 134.637 31.8927L123.671 47.5478L40.3528 164.021L77.1828 206.229C91.2684 222.372 111.997 231.137 133.391 229.998Z"
                        fill="#FFB6AB"
                      ></path>
                      <path
                        className="blob__face"
                        d="M193.676 33.154C191.962 43.1321 201.56 48.0065 208.337 43.7251M216.04 48.6263C214.326 58.6044 223.925 63.4787 230.702 59.1974M179.498 44.6049C182.41 73.6423 202.78 82.5942 226.764 71.9121"
                        stroke="#FF553C"
                        stroke-width="4.92264"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                    </svg>
                  </div>
                  <div className="blobs__arrow">
                    <svg
                      width="201"
                      height="223"
                      viewBox="0 0 201 223"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M115.745 4.20756C107.382 -0.390078 97.2256 -0.284367 88.9609 4.48632L13.6544 47.9557C0.604693 55.4885 -3.86772 72.1739 3.66503 85.2236C11.1978 98.2734 27.8832 102.746 40.9329 95.213L76.8 74.5093L76.8 206.435C76.8 215.197 88.2784 222.3 102.438 222.3C116.597 222.3 128.075 215.197 128.075 206.435L128.075 73.2558L160.413 91.0352C173.617 98.2947 190.205 93.476 197.465 80.2723C204.724 67.0686 199.906 50.4799 186.702 43.2204L115.745 4.20756Z"
                        fill="url(#paint0_linear_36_932)"
                      ></path>
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M115.745 4.20756C107.382 -0.390078 97.2256 -0.284367 88.9609 4.48632L13.6544 47.9557C0.604693 55.4885 -3.86772 72.1739 3.66503 85.2236C11.1978 98.2734 27.8832 102.746 40.9329 95.213L76.8 74.5093L76.8 206.435C76.8 215.197 88.2784 222.3 102.438 222.3C116.597 222.3 128.075 215.197 128.075 206.435L128.075 73.2558L160.413 91.0352C173.617 98.2947 190.205 93.476 197.465 80.2723C204.724 67.0686 199.906 50.4799 186.702 43.2204L115.745 4.20756Z"
                        fill="white"
                      ></path>
                      <defs>
                        <linearGradient
                          id="paint0_linear_36_932"
                          x1="188.471"
                          y1="270.615"
                          x2="-2.11774"
                          y2="132.631"
                          gradientUnits="userSpaceOnUse"
                        >
                          <stop stop-color="white"></stop>
                          <stop offset="1" stop-color="#FFE8F6"></stop>
                        </linearGradient>
                      </defs>
                    </svg>
                  </div>
                </div>
              </div>
            </div>
          </li>
          <li>
            <div className="service text-sm service--purple">
              <div className="service__inner">
                <div className="service__entry">
                  <h3>Cheap Instagram Followers</h3>
                  <p>
                    And by "cheap," we mean we offer high-quality or premium
                    followers at very affordable prices. Our services are
                    cost-effective for an influencer or small business to build
                    a social media presence quickly and effectively.
                  </p>
                </div>
                <div className="offer has-stars">
                  <a className="btn" href="/buy-instagram-followers"
                    ><span></span><span></span><span></span
                    ><strong>Buy Followers</strong></a
                  >
                  <div className="offer__inner">
                    <p>
                      Starting at <strong>$2.97</strong>
                    </p>
                    <div className="stars text-xsm">
                      <ul>
                        <li>
                          <svg
                            className="svg-icon svg-icon--star"
                            width="19"
                            height="20"
                            viewBox="0 0 19 20"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                          >
                            <path
                              d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                              fill="#FF553C"
                            ></path>
                          </svg>
                        </li>
                        <li>
                          <svg
                            className="svg-icon svg-icon--star"
                            width="19"
                            height="20"
                            viewBox="0 0 19 20"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                          >
                            <path
                              d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                              fill="#FF553C"
                            ></path>
                          </svg>
                        </li>
                        <li>
                          <svg
                            className="svg-icon svg-icon--star"
                            width="19"
                            height="20"
                            viewBox="0 0 19 20"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                          >
                            <path
                              d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                              fill="#FF553C"
                            ></path>
                          </svg>
                        </li>
                        <li>
                          <svg
                            className="svg-icon svg-icon--star"
                            width="19"
                            height="20"
                            viewBox="0 0 19 20"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                          >
                            <path
                              d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                              fill="#FF553C"
                            ></path>
                          </svg>
                        </li>
                        <li>
                          <svg
                            className="svg-icon svg-icon--star"
                            width="19"
                            height="20"
                            viewBox="0 0 19 20"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                          >
                            <path
                              d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                              fill="#FF553C"
                            ></path>
                          </svg>
                        </li>
                      </ul>
                    </div>
                    <div className="stars text-xsm">
                      <ul></ul>
                      <strong className="text-orange"
                        >5
                        / 5</strong
                      >
                      — based on
                      4
                      reviews
                    </div>
                  </div>
                </div>
                <svg
                  className="svg-icon svg-icon--shield"
                  width="40"
                  height="49"
                  viewBox="0 0 40 49"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <ellipse
                    cx="20.5"
                    cy="20.0078"
                    rx="15.5"
                    ry="15"
                    fill="#FF553C"
                  ></ellipse>
                  <path
                    d="M20 0.0078125L0 5.83327V14.7792C0 30.4512 7.38909 38.455 20 48.0078C32.611 38.4493 40 30.4512 40 14.7792V5.83327L20 0.0078125ZM17.9677 32.1714L8.16168 22.5862L12.4318 18.3476L17.9616 23.7526L29.3834 12.5861L33.6592 16.8246L17.9677 32.1714Z"
                    fill="#FFB6AB"
                  ></path>
                </svg>
              </div>
              <div className="blobs blobs--purple">
                <div className="blobs__holder">
                  <div className="blob blob--left">
                    <svg
                      width="274"
                      height="238"
                      viewBox="0 0 274 238"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        className="blob__body"
                        d="M143.663 232.92C121.673 233.098 100.832 223.313 87.1516 206.389L18.114 120.983C-6.45971 90.5833 -1.31022 46.3171 29.6158 22.1119C60.5418 -2.09334 105.533 2.92843 130.107 33.3283L142.312 48.4274L234.899 160.655L199.78 205.48C186.349 222.623 165.654 232.743 143.663 232.92Z"
                        fill="#FFB6AB"
                      ></path>
                      <path
                        className="blob__face"
                        d="M41.3622 63.1121C50.1713 68.102 58.0068 60.72 56.272 52.8938M63.4911 47.3046C72.3002 52.2945 80.1356 44.9125 78.4008 37.0863M37.5951 78.3983C65.9042 85.4868 95.6536 65.5285 93.7198 39.3447"
                        stroke="#FF553C"
                        stroke-width="4.92264"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                    </svg>
                  </div>
                  <div className="blob blob--middle">
                    <svg
                      width="349"
                      height="340"
                      viewBox="0 0 349 340"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        className="blob__body"
                        d="M105.043 246.657C87.2405 232.715 77.1338 229.278 76.529 207.517L73.7357 104.671C72.6492 65.5833 104.002 34.8709 143.764 36.0731C183.526 37.2753 216.64 69.9369 217.726 109.025L218.007 121.47L222.873 249.546L166.265 240.452C144.921 245.024 122.846 260.599 105.043 246.657Z"
                        fill="#FFB6AB"
                      ></path>
                      <path
                        className="blob__face"
                        d="M125.413 62.6623C128.746 69.987 137.234 68.9011 139.775 63.0579M147.029 62.8369C150.362 70.1616 158.85 69.0758 161.391 63.2326M111.251 68.8341C126.143 86.6195 165.657 85.5081 176.648 67.7673"
                        stroke="#FF553C"
                        stroke-width="3.91287"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                    </svg>
                  </div>
                  <div className="blob blob--right">
                    <svg
                      width="267"
                      height="234"
                      viewBox="0 0 267 234"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        className="blob__body"
                        d="M133.391 229.998C154.784 228.858 174.464 217.94 186.755 200.392L248.779 111.841C270.856 80.3216 263.202 36.8729 231.682 14.7958C200.163 -7.28138 156.714 0.373153 134.637 31.8927L123.671 47.5478L40.3528 164.021L77.1828 206.229C91.2684 222.372 111.997 231.137 133.391 229.998Z"
                        fill="#FFB6AB"
                      ></path>
                      <path
                        className="blob__face"
                        d="M193.676 33.154C191.962 43.1321 201.56 48.0065 208.337 43.7251M216.04 48.6263C214.326 58.6044 223.925 63.4787 230.702 59.1974M179.498 44.6049C182.41 73.6423 202.78 82.5942 226.764 71.9121"
                        stroke="#FF553C"
                        stroke-width="4.92264"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                    </svg>
                  </div>
                  <div className="blobs__arrow">
                    <svg
                      width="201"
                      height="223"
                      viewBox="0 0 201 223"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M115.745 4.20756C107.382 -0.390078 97.2256 -0.284367 88.9609 4.48632L13.6544 47.9557C0.604693 55.4885 -3.86772 72.1739 3.66503 85.2236C11.1978 98.2734 27.8832 102.746 40.9329 95.213L76.8 74.5093L76.8 206.435C76.8 215.197 88.2784 222.3 102.438 222.3C116.597 222.3 128.075 215.197 128.075 206.435L128.075 73.2558L160.413 91.0352C173.617 98.2947 190.205 93.476 197.465 80.2723C204.724 67.0686 199.906 50.4799 186.702 43.2204L115.745 4.20756Z"
                        fill="url(#paint0_linear_36_932)"
                      ></path>
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M115.745 4.20756C107.382 -0.390078 97.2256 -0.284367 88.9609 4.48632L13.6544 47.9557C0.604693 55.4885 -3.86772 72.1739 3.66503 85.2236C11.1978 98.2734 27.8832 102.746 40.9329 95.213L76.8 74.5093L76.8 206.435C76.8 215.197 88.2784 222.3 102.438 222.3C116.597 222.3 128.075 215.197 128.075 206.435L128.075 73.2558L160.413 91.0352C173.617 98.2947 190.205 93.476 197.465 80.2723C204.724 67.0686 199.906 50.4799 186.702 43.2204L115.745 4.20756Z"
                        fill="white"
                      ></path>
                      <defs>
                        <linearGradient
                          id="paint0_linear_36_932"
                          x1="188.471"
                          y1="270.615"
                          x2="-2.11774"
                          y2="132.631"
                          gradientUnits="userSpaceOnUse"
                        >
                          <stop stop-color="white"></stop>
                          <stop offset="1" stop-color="#FFE8F6"></stop>
                        </linearGradient>
                      </defs>
                    </svg>
                  </div>
                </div>
              </div>
            </div>
          </li>
          <li>
            <div className="service text-sm service--yellow">
              <div className="service__inner">
                <div className="service__entry">
                  <h3>Instagram Views Packages</h3>
                  <p>
                    Instagram video views, Instagram profile views, Instagram
                    story views – fast growth in all of these metrics will bring
                    increased credibility to your account and will "spillover"
                    to provide even more exposure on other social media
                    platforms.
                  </p>
                </div>
                <div className="offer has-stars">
                  <a className="btn" href="/buy-instagram-views"
                    ><span></span><span></span><span></span
                    ><strong>Buy Views</strong></a
                  >
                  <div className="offer__inner">
                    <p>
                      Starting at <strong>$1.99</strong>
                    </p>
                    <div className="stars text-xsm">
                      <ul>
                        <li>
                          <svg
                            className="svg-icon svg-icon--star"
                            width="19"
                            height="20"
                            viewBox="0 0 19 20"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                          >
                            <path
                              d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                              fill="#FF553C"
                            ></path>
                          </svg>
                        </li>
                        <li>
                          <svg
                            className="svg-icon svg-icon--star"
                            width="19"
                            height="20"
                            viewBox="0 0 19 20"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                          >
                            <path
                              d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                              fill="#FF553C"
                            ></path>
                          </svg>
                        </li>
                        <li>
                          <svg
                            className="svg-icon svg-icon--star"
                            width="19"
                            height="20"
                            viewBox="0 0 19 20"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                          >
                            <path
                              d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                              fill="#FF553C"
                            ></path>
                          </svg>
                        </li>
                        <li>
                          <svg
                            className="svg-icon svg-icon--star"
                            width="19"
                            height="20"
                            viewBox="0 0 19 20"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                          >
                            <path
                              d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                              fill="#FF553C"
                            ></path>
                          </svg>
                        </li>
                        <li>
                          <svg
                            className="svg-icon svg-icon--star"
                            width="19"
                            height="20"
                            viewBox="0 0 19 20"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                          >
                            <path
                              d="M8.80729 0.655781C9.19147 -0.218594 9.8084 -0.218594 10.193 0.655781L12.6447 6.22902L18.1772 7.06861C19.0648 7.20264 19.2653 7.86154 18.6265 8.54377L14.588 12.8227L15.5787 18.9209C15.7303 19.8773 15.216 20.2677 14.4315 19.8067L9.50003 16.8973L4.56876 19.8067C3.78387 20.2677 3.26973 19.8773 3.42116 18.9209L4.41182 12.8227L0.373554 8.54377C-0.265227 7.86154 -0.0649666 7.20264 0.822905 7.06861L6.35495 6.22902L8.80729 0.655781Z"
                              fill="#FF553C"
                            ></path>
                          </svg>
                        </li>
                      </ul>
                    </div>
                    <div className="stars text-xsm">
                      <ul></ul>
                      <strong className="text-orange"
                        >5
                        / 5</strong
                      >
                      — based on
                      3
                      reviews
                    </div>
                  </div>
                </div>
                <svg
                  className="svg-icon svg-icon--shield"
                  width="40"
                  height="49"
                  viewBox="0 0 40 49"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <ellipse
                    cx="20.5"
                    cy="20.0078"
                    rx="15.5"
                    ry="15"
                    fill="#FF553C"
                  ></ellipse>
                  <path
                    d="M20 0.0078125L0 5.83327V14.7792C0 30.4512 7.38909 38.455 20 48.0078C32.611 38.4493 40 30.4512 40 14.7792V5.83327L20 0.0078125ZM17.9677 32.1714L8.16168 22.5862L12.4318 18.3476L17.9616 23.7526L29.3834 12.5861L33.6592 16.8246L17.9677 32.1714Z"
                    fill="#FFB6AB"
                  ></path>
                </svg>
              </div>
              <div className="blobs blobs--yellow">
                <div className="blobs__holder">
                  <div className="blob blob--left">
                    <svg
                      width="274"
                      height="238"
                      viewBox="0 0 274 238"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        className="blob__body"
                        d="M143.663 232.92C121.673 233.098 100.832 223.313 87.1516 206.389L18.114 120.983C-6.45971 90.5833 -1.31022 46.3171 29.6158 22.1119C60.5418 -2.09334 105.533 2.92843 130.107 33.3283L142.312 48.4274L234.899 160.655L199.78 205.48C186.349 222.623 165.654 232.743 143.663 232.92Z"
                        fill="#FFB6AB"
                      ></path>
                      <path
                        className="blob__face"
                        d="M41.3622 63.1121C50.1713 68.102 58.0068 60.72 56.272 52.8938M63.4911 47.3046C72.3002 52.2945 80.1356 44.9125 78.4008 37.0863M37.5951 78.3983C65.9042 85.4868 95.6536 65.5285 93.7198 39.3447"
                        stroke="#FF553C"
                        stroke-width="4.92264"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                    </svg>
                  </div>
                  <div className="blob blob--middle">
                    <svg
                      width="349"
                      height="340"
                      viewBox="0 0 349 340"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        className="blob__body"
                        d="M105.043 246.657C87.2405 232.715 77.1338 229.278 76.529 207.517L73.7357 104.671C72.6492 65.5833 104.002 34.8709 143.764 36.0731C183.526 37.2753 216.64 69.9369 217.726 109.025L218.007 121.47L222.873 249.546L166.265 240.452C144.921 245.024 122.846 260.599 105.043 246.657Z"
                        fill="#FFB6AB"
                      ></path>
                      <path
                        className="blob__face"
                        d="M125.413 62.6623C128.746 69.987 137.234 68.9011 139.775 63.0579M147.029 62.8369C150.362 70.1616 158.85 69.0758 161.391 63.2326M111.251 68.8341C126.143 86.6195 165.657 85.5081 176.648 67.7673"
                        stroke="#FF553C"
                        stroke-width="3.91287"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                    </svg>
                  </div>
                  <div className="blob blob--right">
                    <svg
                      width="267"
                      height="234"
                      viewBox="0 0 267 234"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        className="blob__body"
                        d="M133.391 229.998C154.784 228.858 174.464 217.94 186.755 200.392L248.779 111.841C270.856 80.3216 263.202 36.8729 231.682 14.7958C200.163 -7.28138 156.714 0.373153 134.637 31.8927L123.671 47.5478L40.3528 164.021L77.1828 206.229C91.2684 222.372 111.997 231.137 133.391 229.998Z"
                        fill="#FFB6AB"
                      ></path>
                      <path
                        className="blob__face"
                        d="M193.676 33.154C191.962 43.1321 201.56 48.0065 208.337 43.7251M216.04 48.6263C214.326 58.6044 223.925 63.4787 230.702 59.1974M179.498 44.6049C182.41 73.6423 202.78 82.5942 226.764 71.9121"
                        stroke="#FF553C"
                        stroke-width="4.92264"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                    </svg>
                  </div>
                  <div className="blobs__arrow">
                    <svg
                      width="201"
                      height="223"
                      viewBox="0 0 201 223"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M115.745 4.20756C107.382 -0.390078 97.2256 -0.284367 88.9609 4.48632L13.6544 47.9557C0.604693 55.4885 -3.86772 72.1739 3.66503 85.2236C11.1978 98.2734 27.8832 102.746 40.9329 95.213L76.8 74.5093L76.8 206.435C76.8 215.197 88.2784 222.3 102.438 222.3C116.597 222.3 128.075 215.197 128.075 206.435L128.075 73.2558L160.413 91.0352C173.617 98.2947 190.205 93.476 197.465 80.2723C204.724 67.0686 199.906 50.4799 186.702 43.2204L115.745 4.20756Z"
                        fill="url(#paint0_linear_36_932)"
                      ></path>
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M115.745 4.20756C107.382 -0.390078 97.2256 -0.284367 88.9609 4.48632L13.6544 47.9557C0.604693 55.4885 -3.86772 72.1739 3.66503 85.2236C11.1978 98.2734 27.8832 102.746 40.9329 95.213L76.8 74.5093L76.8 206.435C76.8 215.197 88.2784 222.3 102.438 222.3C116.597 222.3 128.075 215.197 128.075 206.435L128.075 73.2558L160.413 91.0352C173.617 98.2947 190.205 93.476 197.465 80.2723C204.724 67.0686 199.906 50.4799 186.702 43.2204L115.745 4.20756Z"
                        fill="white"
                      ></path>
                      <defs>
                        <linearGradient
                          id="paint0_linear_36_932"
                          x1="188.471"
                          y1="270.615"
                          x2="-2.11774"
                          y2="132.631"
                          gradientUnits="userSpaceOnUse"
                        >
                          <stop stop-color="white"></stop>
                          <stop offset="1" stop-color="#FFE8F6"></stop>
                        </linearGradient>
                      </defs>
                    </svg>
                  </div>
                </div>
              </div>
            </div>
          </li>
        </ul>
      </div>
    </div>
  </div>
</div>
      
    </div>
  )
}
